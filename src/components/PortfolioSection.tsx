...
const portfolioItems = [{
  ...
  images: ["https://images.unsplash.com/photo-1563013544-824ae1b704d3?w=800&h=600&fit=crop", "https://images.unsplash.com/photo-1611224923853-80b023f02d71?w=800&h=600&fit=crop", "https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800&h=600&fit=crop"],
  tags: ["React Native", "Node.js", "MongoDB", "Stripe"],
  technologies: ["React Native", "TypeScript", "Node.js", "Express", "MongoDB", "Stripe API", "JWT", "Socket.io"],
  features: ["Real-time market data and analytics", "Biometric authentication and security", "AI-powered spending insights", "Investment portfolio tracking", "Budget optimization algorithms", "Seamless bank integrations"],
  challenges: ["Implementing real-time data synchronization across devices", "Ensuring bank-level security standards", "Creating intuitive UX for complex financial data", "Optimizing performance for large datasets"],
...
export const PortfolioSection = () => {
  ...
  return <section className="py-24 px-4 relative overflow-hidden bg-muted/20">
      {/* Background Elements */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {portfolioItems.map((item, index) => <ScrollAnimation key={item.title} animationType="scale-in" delay={index * 100}>
              <Card className="group hover-lift cursor-pointer" onClick={() => handleViewProject(item)}>
              <CardContent className="p-0">
                {/* Project Image */}
                <div className="relative overflow-hidden rounded-t-xl">
                  <img src={item.image} alt={item.title} className="w-full h-48 object-cover transition-smooth group-hover:scale-110" />
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {item.tags.map(tag => <span key={tag} className="px-2 py-1 bg-muted text-xs rounded-md text-muted-foreground">
                        {tag}
                      </span>)}
                  </div>
        ...
        <ScrollAnimation animationType="fade-up" className="text-center">
          <Card className="max-w-4xl mx-auto p-12">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button variant="hero" size="xl" className="group" onClick={handleViewFullPortfolio}>
                View Full Portfolio
        ...
        <PortfolioModal project={selectedProject} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
      </div>
    </section>;
};